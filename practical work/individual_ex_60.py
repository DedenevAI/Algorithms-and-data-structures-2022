...
def start(message):
   ...
   bot.send_message(message.chat.id, "👋 Привет, {0.first_name}! Я Ваш бот-помошник! Я осуществляю поиск статьи по ключевой фразе, получаю URL-ссылки статей и формирую из них файл, который отправляю Вам!".format(message.from_user), reply_markup = markup)
